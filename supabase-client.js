...
async function votePoll(id, optionIndex) {
    try {
        ...
        // String olarak gelen optionIndex'i sayıya çevir ve sonra tekrar string'e çevir
        // Bu şekilde tutarlılık sağlanır
        const optionKey = parseInt(optionIndex).toString();
        
        // Debug log ekleyelim
        console.log('Oy verilecek anket:', id);
        console.log('Oy verilecek seçenek indeksi:', optionKey);
        console.log('Mevcut oylar:', JSON.stringify(votes));
        
        // Oy sayısını artır
        votes[optionKey] = (votes[optionKey] || 0) + 1;
        
        console.log('Güncellenmiş oylar:', JSON.stringify(votes));
        ...
        // Veritabanından güncellenmiş anketi tekrar alalım
        const { data: updatedPoll, error: refreshError } = await supabase
            .from('polls')
            .select('*')
            .eq('id', id)
            .single();
        ...
        if (refreshError) {
            console.error('Güncellenmiş anketi alma hatası:', refreshError);
            throw refreshError;
        }
        
        console.log('Veritabanından güncellenmiş anket:', updatedPoll);
        console.log('Güncellenmiş oylar (doğrulama):', JSON.stringify(updatedPoll.votes));
...
// Audio dosyası yükleme fonksiyonu
async function uploadAudio(file, path = 'audio') {
    // Hata kayıtları için daha detaylı bilgiler
    console.log(`Audio yükleme başlatıldı - Dosya bilgileri:`, { 
    ...
    try {
        ...
        if (!file) {
            console.error('HATA: Audio dosyası bulunamadı veya boş');
            showErrorMessage('Lütfen bir ses dosyası seçin.');
            return null;
        }
        ...
        if (!file.type || !file.type.startsWith('audio/')) {
            ...
            showErrorMessage('Lütfen geçerli bir ses dosyası seçin (MP3, WAV, OGG).');
            return null;
        }
        ...
        if (file.size < 100) { // 100 byte'dan küçük sesler muhtemelen bozuk
        ...
        try {
            ...
            else if (file.type === 'audio/wav') fileExtension = 'wav';
            else if (file.type === 'audio/ogg') fileExtension = 'ogg';
            else fileExtension = 'mp3'; // Varsayılan
        }
        ...
        try {
            ...
            if (error) {
                ...
                try {
                    ...
                    console.log('Audio dosyası başarıyla yüklendi (2. deneme):', urlData.publicUrl);
            ...
            console.log('Audio dosyası başarıyla yüklendi:', urlData.publicUrl);
            
            // Loading göstergesini kapat ve başarı mesajı göster
            hideLoadingIndicator();
            showSuccessMessage('Audio başarıyla yüklendi!');
